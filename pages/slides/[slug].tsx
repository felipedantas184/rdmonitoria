...
import { SlidesData } from "@/components/Curso/CursoData"
...
export const getStaticPaths = async () => {
  const paths = SlidesData.map(item => {
    return {
...
export const getStaticProps = async ({ params }: any) => {
  const item = SlidesData.filter((item: any) => item.slug == params.slug)
...
export default function PDFPage({lista}:any) {
  ...
  return (
    <>
      ...
      <Layout>
        {(!loading) && (
          <PDFViewer lista={lista} access={userData?.course} />
        )}
      </Layout>
    </>
  );
